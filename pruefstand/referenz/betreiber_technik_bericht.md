...
### 7.0 Datei-Inventar (Inode und Zeitstempel sichern)
  ...
  Inventar: 670 Datei(en) erfasst, davon 670 mit Anlegezeit (belege/00_dateien.tsv)
...
<INODE> -rw-r--r-- <EIGNER> 102 <MTIME> <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/upgrade/wp_pruefstand2/wordpress/wp-includes/pruefstand-gemischt.php
<INODE> -rw-r--r-- <EIGNER> 102 <MTIME> <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-includes/pruefstand-gemischt.php
...
<INODE> -rw-r--r-- <EIGNER> 3466 <MTIME> <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/uploads/cache/wpml/twig/3e/3e63fcbd19fd358f040b36a0ac02099.php
...
- ⚠️  **3 PHP-Datei(en) in Upload-Verzeichnissen sind erzeugter Code bekannten Formats — nicht in Quarantäne, aber zu sichten**

```
TCPDF-Schriftmetrik  <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/uploads/wcj_uploads/tcpdf_fonts/stsongstdlight.php
WPML-Twig-Cache  <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/uploads/cache/wpml/twig/3e/3e63fcbd19fd358f040b36a0ac02099.php
Datenablage hinter __halt_compiler()  <PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/uploads/aios/firewall-rules/settings.php
  
```

  Beleg: belege/002_php_in_uploads_erzeugt.txt
- 🔴 **KRITISCH: PHP-Dateien in Upload-Verzeichnissen (nach Guard-Filter, extrem verdächtig; 6 Guard-/Plugin-Dateien gefiltert)**

```
<PRUEFSTAND>/vhosts/kunde-zwei.example/httpdocs/wp-content/uploads/cache/wpml/twig/3e/getarnt.php
...
```
  ...
  Beleg: belege/003_php_in_uploads_mit_hashes.txt
...
```
  ...
  Beleg: belege/004_htaccess_weiterleitungen.txt
...
```
  ...
  Beleg: belege/005_htaccess_einzelfreigabe_php.txt
...
```
  ...
  Beleg: belege/006_htaccess_fremdname_vorhanden.txt
...
- ⚪ **Nicht messbar: YARA-Signaturscan nicht gelaufen (Schalter --yara nicht gesetzt) — diese Erkennungsebene fehlt in diesem Bericht**
...
```
  ...
  Beleg: belege/007_php_in_mediendateien.txt
...
### 7.15 Injektion in grosse Dateien (ohne Referenz)
  ...
  10 Datei(en) mit auffälliger Verteilung — Einordnung in Abschnitt 13d
...
```
Datei-Editor im Backend aktiv (DISALLOW_FILE_EDIT nicht gesetzt) — wer ein Adminkonto hat, schreibt PHP ohne Dateizugriff
wp-config.php ist weltlesbar (644) — sie enthält die Datenbank-Zugangsdaten

```

  Beleg: belege/012_wp_haertung_wpconfig_kunde-drei_example_httpdocs.txt
...
- ✅ kunde-eins.example/httpdocs: keine Application Passwords vergeben
- ⚪ **Nicht messbar: kunde-eins.example/httpdocs: REST-Benutzerliste nicht geprüft (ohne --online) — /wp-json/wp/v2/users liefert unangemeldet oft alle Benutzernamen**
- ⚠️  **kunde-eins.example/httpdocs: 2 Härtungspunkt(e) in wp-config.php offen**
...
```
  ...
  Beleg: belege/013_wp_haertung_wpconfig_kunde-eins_example_httpdocs.txt
...
- ✅ kunde-vier.example/httpdocs: keine Application Passwords vergeben
- ⚪ **Nicht messbar: kunde-vier.example/httpdocs: REST-Benutzerliste nicht geprüft (ohne --online) — /wp-json/wp/v2/users liefert unangemeldet oft alle Benutzernamen**
- ⚠️  **kunde-vier.example/httpdocs: 2 Härtungspunkt(e) in wp-config.php offen**
...
```
  ...
  Beleg: belege/014_wp_haertung_wpconfig_kunde-vier_example_httpdocs.txt
...
- ⚠️  **kunde-zwei.example/httpdocs: 1 Verzeichnis(se) unter plugins/ mit PHP-Dateien, aber ohne Plugin-Kopf — kein Plugin; kann eine Angreifer-Ablage sein, sichten**
  Beleg: belege/015_wp_plugins_ohne_kopf_mit_php_kunde-zwei_example_httpdocs.txt
  ...
  Beleg: belege/016_wp_plugins_datenordner_kunde-zwei_example_httpdocs.txt
...
- ⚪ **Nicht messbar: kunde-zwei.example/httpdocs: 2 Bestandteil(e) ohne lesbare Fassung — für sie ist keine Aussage zur Angreifbarkeit möglich**
  Beleg: belege/017_wp_version_nicht_bewertbar_kunde-zwei_example_httpdocs.txt
  Beleg: belege/018_wp_schwachstellen_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/019_wp_robots_doorway_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/020_wp_include_injektion_kunde-zwei_example_httpdocs.txt
...
- ⚪ **Nicht messbar: kunde-zwei.example/httpdocs: 1 Plugin(s) ohne Prüfsummensatz und alle Themes — Unversehrtheit nicht feststellbar (Premium, Fork, Eigenbau; für Themes veröffentlicht wordpress.org keine Prüfsummen)**
  Beleg: belege/024_wp_ohne_pruefsummen_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/025_wp_staging_veraendert_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/026_wp_core_veraendert_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/027_wp_core_gemischt_kunde-zwei_example_httpdocs.txt
- ⚠️  **kunde-zwei.example/httpdocs: 1 Core-fremde Datei(en) in wp-admin/wp-includes — prüfen**
  Beleg: belege/028_wp_core_fremd_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/029_wp_haertung_wpconfig_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/030_wordfence_uebersprungen_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/031_wp_db_plugin_leichen_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/032_wp_db_optionen_php_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/033_wp_db_optionen_gross_kunde-zwei_example_httpdocs.txt
...
```
  ...
  Beleg: belege/034_htaccess_fremd_kunde-zwei_example_backups_updater-abc123_nextcloud-28_0_1_2-1700000000__htaccess.txt
...
```
  ...
  Beleg: belege/035_htaccess_fremd_kunde-zwei_example_cloud_kunde-zwei_example__htaccess.txt
...
```
  ...
  Beleg: belege/036_htaccess_fremd_kunde-zwei_example_httpdocs__htaccess.txt
...
```
  ...
  Beleg: belege/037_htaccess_fremd_kunde-zwei_example_httpdocs_wp-content_uploads__htaccess.txt
  Beleg: belege/038_htaccess_einordnung.txt
...
```
  ...
  Beleg: belege/039_php_malware_finder_treffer.txt
...
### 13d Einordnung der Mustertreffer aus 7.3 und der Rangfolge aus 7.15
  ...
  Keine Einordnung der Mustertreffer möglich — die Mustersuche in 7.3 ist nicht gelaufen
  4 grosse Datei(en) mit auffälliger Verteilung — nach Punkten sortiert, Einordnung offen (6 gegen amtliche Prüfsummen entlastet)
...
<PRUEFSTAND>/vhosts/kunde-vier.example/httpdocs/wp-includes/gross-kern-ungeprueft.php	4	DICHTE=120,RANDLAGE
...
```
  ...
  Beleg: belege/040_injektion_grosse_dateien.txt
  Beleg: belege/041_injektion_entlastet.txt
...
```
  ...
  Beleg: belege/042_ursache_zeitachse.txt
...
```
  ...
  Beleg: belege/043_ursache_zeitstempel_deutung.txt
...
```
  ...
  Beleg: belege/044_ursache_quellenlage.txt

## 14. ZUSAMMENFASSUNG

- ✅ Programmstand über den ganzen Lauf unverändert — die Fassungsangabe belegt diesen Lauf
  Fundstellen-Details: <PRUEFSTAND>/ablage/forensik/<LAUF-ID>/kunde/befunde_details.md (30 Fund(e), 7 Familien, 1 zu prüfen)
...
> - YARA-Signaturscan nicht gelaufen (Schalter --yara nicht gesetzt) — diese Erkennungsebene fehlt in diesem Bericht
...
> - kunde-drei.example/httpdocs: REST-Benutzerliste nicht geprüft (ohne --online) — /wp-json/wp/v2/users liefert unangemeldet oft alle Benutzernamen
