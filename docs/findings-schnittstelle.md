...
| `actionable.htaccess_fremdname` | Dateien, die in einer `.htaccess` **namentlich** freigegeben sind und dort auch liegen. Die Liste mit der höchsten Trennschärfe im Werkzeug — auf 471 Domains 37 Treffer, davon 29 Kopien derselben Verbreiter-Shell (NT-Forensik #56). **Wird seit NT-Repair 0.8.1 quarantänisiert.** Sie fehlte dort einen Tag lang, weil ein neuer Schlüssel die Bereinigung nicht von selbst erreicht — dagegen prüft jetzt `Repair/werkzeuge/schnittstelle-pruefstand.sh`. |
...
| `actionable.wp_db_optionen_php` | `[{pfad, eintrag}]` — Optionen mit PHP-Merkmal. **Kein Befund, nur Rangfolge für die Sichtung.** Auf 121 echten Installationen: 12 Treffer, 0 echte — legitime Plugins legen dort Code ab (`simplehooks-settings` 6×, Wordfence-Schwachstellenbestand 1×). Der Eintrag führt seit v3.16 das getroffene Muster, die Wertlänge und ein Fenster um den Treffer. **Die Bereinigung liest die Liste nicht.** |
| `actionable.wp_gruender_admins` | Administratoren, die ihre Installation **eröffnet** haben — kein Benutzer der Instanz ist älter. Je Eintrag `{pfad, benutzer, email, angelegt}`. **Kein Bereinigungsziel.** Eine nach dem Vorfall angelegte Seite kann keinen „nachträglich hinzugefügten" Admin haben; ihr erster Benutzer ist der Installateur. Am 14.08.2026 hätte die Bereinigung sonst auf zwei laufenden Kundenseiten den einzigen Administrator ausgesperrt. |
| `actionable.php_in_uploads_erzeugt` | Erzeugter Code bekannten Formats unter `uploads/` — WPML-Twig-Cache, TCPDF-Schriftmetriken, Datenablagen hinter `__halt_compiler()`. Je Eintrag `{pfad, eintrag}`, wobei `eintrag` den Erzeuger nennt. Gemessen am 14.08.2026: 236 von 282 Treffern. **Kein Quarantäne-Kandidat**, aber zu sichten — ein Twig-Cache kann Code ausführen. Erkannt wird das FORMAT, nicht der Ort. |
| `actionable.zerlegte_funktionsnamen` | Dateien, die Funktionsnamen aus Einzelzeichen zusammensetzen (`"f"."o"."p"."e"."n"`). Gemessen auf 6,4 Mio Pfaden: 5 Treffer, 0 Fehlalarme — darunter eine Webshell, die §7.3, die Prüfsummen, §7.6b, Imunify und 13e.2 überstanden hatte. **Quarantäne-Kandidat.** |
| `actionable.webshell_dropper_vendor` / `metrics.webshell_vendor` | Mustertreffer aus Abhängigkeitsverzeichnissen (`vendor/`, `vendor-prefixed/`, `node_modules/`). Bewusst **getrennt von** `webshell_dropper`, weil NT-Repair daraus seine Quarantäne-Kandidaten zieht — auf dem echten Server waren 156 von 292 Einträgen Bibliothekscode (#46). Zur Sichtung, nicht zur Quarantäne. |
| `actionable.wp_configs` | Pfade aller `wp-config.php`. War nach dem Umzug nach `rezepte/` dauerhaft **leer** — `nt_repair.sh:658` liest daraus die Rotationsziele, und die DSGVO-Meldung ihre betroffenen Datenbanken. Beides lief ins Leere. Seit v3.16 wieder gefüllt; die CI sichert `len(wp_configs) == metrics.wp_installs` zu. |
| `run.programmstand_stabil` | `false`, wenn sich der Programmcode **während** des Laufs geändert hat oder nicht feststellbar war. Dann stammen die Abschnitte aus verschiedenen Fassungen und `tool_version` belegt diesen Lauf nicht — im Fehlerfall stehen zusätzlich `run.programmstand_vorher` und `…_nachher` (Commit-Kurzform). Sind beide gleich, wurde ohne git ausgerollt (`cp`, rsync). Wer Befunde archiviert oder gegen einen Stand nachvollzieht, muss dieses Feld lesen (#55). |
